...
<head>
  ...
  <title>JaceTrack - Account Deletion Requests</title>
...
<body>
  ...
  <div class="container py-4">
    <h4 class="fw-bold mb-3 text-primary"><i class="bx bx-user-x me-2"></i>Account Deletion Requests</h4>
            ...
            <tbody>
              @forelse($requests as $req)
                <tr>
                  ...
                  <td>
                    @if($req->option === '7_days')
                      <span class="badge bg-info">7 Days</span>
                    @else
                      <span class="badge bg-warning text-dark">Immediate</span>
                    @endif
                  </td>
                  <td>
                  ...
                  <td>
                    @if($req->scheduled_deletion)
                      {{ \Carbon\Carbon::parse($req->scheduled_deletion)->format('M d, Y h:i A') }}
                    @else
                  ...
                  <td>
                    @if($req->status === 'pending')
                      <form action="{{ route('admin.deletion.approve', $req->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PUT')
                        <button class="btn btn-success btn-sm"><i class="bx bx-check"></i> Approve</button>
                      </form>
                      <form action="{{ route('admin.deletion.reject', $req->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('PUT')
              ...
              @empty
                <tr>
                  <td colspan="8" class="text-muted">No deletion requests found.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
